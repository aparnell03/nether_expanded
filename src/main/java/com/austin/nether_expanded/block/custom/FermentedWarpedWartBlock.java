        ...
        ItemStack itemStack = player2.getStackInHand(hand);
            ...
            if (itemStack.isOf(Items.GLASS_BOTTLE)) {
                ...
                if (itemStack.isEmpty()) {
                    player2.setStackInHand(hand, new ItemStack(ModItems.WARP_JUICE_BOTTLE));
                } else if (!player2.getInventory().insertStack(new ItemStack(ModItems.WARP_JUICE_BOTTLE))) {
                    player2.dropItem(new ItemStack(ModItems.WARP_JUICE_BOTTLE), false);
                }
                world.emitGameEvent((Entity)player2, GameEvent.FLUID_PICKUP, pos);
                world.setBlockState(pos, pushEntitiesUpBeforeBlockChange(state, Blocks.WARPED_WART_BLOCK.getDefaultState(), world, pos));
            }
            if (!world.isClient()) {
