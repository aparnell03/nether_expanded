...
import com.austin.nether_expanded.item.custom.FortressCompassItem;
...
public class ModModelPredicateProvider {
    public static void registerModels(){
        registerFortressCompass();
    }

    private static void registerFortressCompass(){
        FabricModelPredicateProviderRegistry.register(ModItems.FORTRESS_COMPASS, new Identifier("angle"), new UnclampedModelPredicateProvider(){
            ...
            public float unclampedCall(ItemStack itemStack, @Nullable ClientWorld clientWorld, @Nullable LivingEntity livingEntity, int i) {
                ...
                BlockPos blockPos = FortressCompassItem.hasFortress(itemStack) ? this.getFortressPos(clientWorld, itemStack.getOrCreateNbt()) : null;
            ...
            @Nullable
            private BlockPos getFortressPos(World world, NbtCompound nbt) {
                Optional<RegistryKey<World>> optional;
                boolean bl = nbt.contains("FortressPos");
                boolean bl2 = nbt.contains("FortressDimension");
                if (bl && bl2 && (optional = FortressCompassItem.getFortressDimension(nbt)).isPresent() && world.getRegistryKey() == optional.get()) {
                    return NbtHelper.toBlockPos(nbt.getCompound("FortressPos"));
                }
                return null;
            }
